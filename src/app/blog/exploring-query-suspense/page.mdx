...
>
> <br />
> Changed the way the `isSuspending` flag is attached to the returned query
> object.
>
...
>
> <br />
> This was not optimal as it would disable [render
> optimizations](https://tanstack.com/query/v5/docs/framework/react/guides/render-optimizations#tracked-properties)
> in React Query.
> <br />
> The more optimal way is to attach the `isSuspending` flag directly to the
> query object to maintain the property tracking in React Query.
...
suspends on initial load and shows the loading fallback but on subsequent
fetches the component can use the `isSuspending` flag to show an inline loading
indicator.
...
>
> <br />
> **Suspense for data fetching should only be used for non-dependant data that
> is rendering-critical.**
>
> <br />
> Meaning any data that is not dependant on other data (to avoid fetch
> waterfalls) and is required to render a given view in a functionally complete
> state.
